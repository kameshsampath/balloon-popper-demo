...
-- CREATE STAGE IF NOT EXISTS balloon_silver.sis.balloon_sis_stage;
...
--    snow stage copy snowflake/sis/streamlit_app.py @balloon_silver.sis.balloon_sis_stage/
--    snow stage copy snowflake/sis/environment.yml @balloon_silver.sis.balloon_sis_stage/

-- 3) Create the Streamlit object (files are copied at CREATE time; re-upload + OR REPLACE to update)
-- CREATE OR REPLACE STREAMLIT balloon_silver.sis.balloon_game_dashboard
--   FROM @balloon_silver.sis.balloon_sis_stage
--   MAIN_FILE = 'streamlit_app.py'
--   QUERY_WAREHOUSE = COMPUTE_WH
--   TITLE = 'Balloon pops — silver';

-- 4) Go live (required after CREATE): see "Usage notes" in CREATE STREAMLIT docs.
-- ALTER STREAMLIT balloon_silver.sis.balloon_game_dashboard ADD LIVE VERSION FROM LAST;

-- 5) Grants — readers need USAGE on the Streamlit object and underlying SELECT on silver tables / DTs.
-- GRANT USAGE ON STREAMLIT balloon_silver.sis.balloon_game_dashboard TO ROLE <analyst_role>;
