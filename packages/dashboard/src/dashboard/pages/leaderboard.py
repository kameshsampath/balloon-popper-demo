...
def create_score_chart(data, f_selected_players, f_time_unit: str = "minutes"):
    ...
    aggregated_df = filtered_df.groupby(['window_start', 'player'])['total_score'].max().reset_index()
    ...
    __chart = (lines + points).properties(
        width=700,
        height=400,
        title='Player Total Score Trends Over Time'
    ).interactive()

    return __chart


def filter_data_by_time(df: pd.DataFrame, f_start_time: Timestamp, f_end_time: Timestamp):
...
if st.session_state.leaderboard_data is not None:
    ...
    if st.session_state.realtime_scores_data is not None:
        ...
        with st.sidebar:
            ...
            st.write(f"max_time: {max_time},min_time: {min_time}")
            ...
            with col1:
                start_time = st.time_input(
                    "Start Time",
                    value=pd.to_datetime(min_time).time()
                )
            ...
            with col2:
                end_time = st.time_input(
                    "End Time",
                    value=pd.to_datetime(max_time).time()
                )
